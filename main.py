...
@retry(wait_fixed=2000, stop_max_attempt_number=3, retry_on_exception=retry_if_timeout)
...
def process_file(file_entry, context: CallbackContext) -> None:
    try:
        ...
        output_file_name = "aggregated_sub-domains.txt"
        ...
        with open(output_file_path, 'a') as aggregated_file:
            for domain in new_domains:
                subprocess.check_call([
                    'sublist3r', '-d', domain, '-o', aggregated_file.name, '-v'
                ])
...
def handle_document(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        for domain in domains:
            unique_filename = f"{domain.replace('.', '_')}_sub-domains.txt"
            output_file_path = os.path.join(OUTPUTS_DIR, unique_filename)

            if not processing_now:
                context.bot.send_message(chat_id,
                                         "Please wait. This might take some time!")

            executor.submit(process_single_domain, domain, output_file_path, chat_id, context)
            processing_now = True
...
def process_single_domain(domain, output_file_path, chat_id, context):
    try:
        subprocess.check_call(['sublist3r', '-d', domain, '-o', output_file_path])
